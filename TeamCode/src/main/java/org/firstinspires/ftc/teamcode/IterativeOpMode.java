...
public class IterativeOpMode extends OpMode {
    ...
    public void loop() {
        ...
        {
            robot.flipper.setPower(-gamepad1.right_trigger*0.6);
        }
        else if (gamepad1.left_trigger>0)
        {
            robot.flipper.setPower(gamepad1.left_trigger*0.6);
        }

        double carouselPwr = 0;
        if (gamepad2.right_trigger > 0) {
            carouselPwr = gamepad2.right_trigger*0.5;
        } else if (gamepad2.left_trigger > 0) {
            carouselPwr = -gamepad2.left_trigger*0.5;
        }
        double cascadePwr = 0;
        if (gamepad2.right_stick_y<0)
        {
            cascadePwr=-gamepad2.right_stick_y*0.5;// forward is pos, backward is neg. (-1 <= magnitude <= 1)
        }
        else if (gamepad2.right_stick_y>0)
        {
            cascadePwr=-gamepad2.right_stick_y*0.1;// forward is pos, backward is neg. (-1 <= magnitude <= 1)
        }
        double rotatorPwr = 0;
